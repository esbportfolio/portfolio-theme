...
class Esb_Comment_Walker extends Walker_Comment {
    ...
    private function format_comment_reply_link(WP_Comment $comment): string {
        ...
        return $this->html_helper->create_html_tag(
            tag_type: 'a',
            inner_html: 'Reply',
            classes: array('comment-reply-link', 'text-dark', 'esb-hover-line-only'), // comment-reply-link is what WP JS uses to identify links
            attr: $attr,
        );
    ...
    private function format_comment_edit_link(WP_Comment $comment): string {
        ...
        return $this->html_helper->create_html_tag(
            tag_type: 'a',
            inner_html: 'Edit',
            classes: array('me-3', 'text-dark', 'esb-hover-line-only'),
            attr: array(
                'href' => $href,
                'target' => '_self',
            ),
    ...
    private function format_edit_reply_line(WP_Comment $data_object, int $depth, array $args, bool $user_can_edit): string {
        /**
    ...
    private function format_comment_data(WP_Comment $data_object, int $depth, array $args, bool $user_can_edit, bool $user_owns_comment = false): array {
        ...
        // Set the standard classes for authors, and then add one if the user owns the comment
        $author_classes = array('mb-0', 'fw-bold');
        if ($user_owns_comment) {
            $author_classes[] = 'text-success';
        }
        ...
        if (!$data_object->comment_approved) {
            ...
            $mod_p = $this->html_helper->create_html_tag(
                ...
                classes: array('alert', 'alert-warning', 'mt-1', 'p-1', 'fst-italic'),
            );
        ...
        $edit_reply_line = $this->format_edit_reply_line($data_object, $depth, $args, $user_can_edit);
