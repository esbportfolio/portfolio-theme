...
class Esb_Post_Formatter {
        ...
        if ($items) {
            ...
            foreach($items as $item) {
                ...
                $link_classes = ($item->taxonomy === 'post_tag') ? 
                    ...
                    array('esb-hover-line-only', 'text-muted');
    ...
    private function format_post_data(bool $excerpt_only): array {
        ...
        if ($excerpt_only) {
            $post_read_more = $html_helper->create_html_tag(
                ...
                classes: array('esb-hover-line-none'),
                attr: array('href' => get_permalink())
            );
        };
    ...
    public function format_post(int $base_indent = 0, bool $excerpt_only = false): string {
        ...
        $read_more_line = ($post_data['read_more']) ? str_repeat(T, $base_indent + 2) . '<p>' . $post_data['read_more'] . '</p>' . N : '';
        $tags_line = ($post_data['tags']) ? str_repeat(T, $base_indent + 2) . $post_data['tags'] . N : '';
        $cats_line = ($post_data['cats']) ? str_repeat(T, $base_indent + 2) . $post_data['cats'] . N : '';

        // Build the content that goes inside the card body
        $content_html = 
            str_repeat(T, $base_indent + 2) . $post_data['title'] . N . 
            str_repeat(T, $base_indent + 2) .  $post_data['date'] . N . 
            $tags_line . 
            str_repeat(T, $base_indent + 2) .  $post_data['content'] . 
            $read_more_line . 
            $cats_line;

        // Build output
        $output = 
            str_repeat(T, $base_indent) . '<div class="card mb-3">' . N . 
            str_repeat(T, $base_indent + 1) . '<div class="card-body">' . N . 
            $content_html . 
            str_repeat(T, $base_indent + 1) . '</div>' . N . 
